    ...
    <div class="main">
        ...
        <section id="yes" class="hero">
            <div ref="gridRef" class="grid-system">
                <div v-for="n in 12" :key="n" :class="['block', `block--${n}`, morphDirections[n - 1]]"></div>
                ...
                <div class="block--text">
                    <div ref="heroTextRef" class="hero-text">
                        ...
                        >&nbsp;<span class="here-sentence"></span> <span class="hero-ready"><strong>R</strong><span class="morph">YEONG</span></span
        ...
        <div class="inner">
                ...
                <article class="posts">
                    <div>
                            ...
                            <div class="quick-buttons">
                                <ClientOnly>
                                    ...
                                    <path stroke="currentColor" d="M7 7L17 17M17 17V9M17 17H9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                        ...
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path stroke="currentColor" d="M5 12H19M19 12L13 6M19 12L13 18" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </NuxtLink>
                                </ClientOnly>
                            </div>
                        ...
                        <div class="quick-more">
                            <ClientOnly>
                                <NuxtLink v-hover="{ bg: '#fd30ae', text: '#fff' }" target="_blank" to="https://nebula-fiction-5e4.notion.site/197a784e4dc28023bd15de919529bcdd?v=197a784e4dc280e5aa57000cb639d563" class="quick">
                            ...
                            <ClientOnly>
                                <NuxtLink v-hover="{ bg: '#ce2cff', text: '#fff' }" target="_blank" to="https://www.notion.so/198a784e4dc2809eb19affa4e028e53a?v=1ada784e4dc2804f9ae9000ced9c1d0f&source=copy_link" class="quick">
                                    <svg class="icon" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <title>Notion</title>
                                        <path fill="currentColor" d="M4.459 4.208c.746.606 1.026.56 2.428.466l13.215-.793c.28 0 .047-.28-.046-.326L17.86 1.968c-.42-.326-.981-.7-2.055-.607L3.01 2.295c-.466.046-.56.28-.374.466zm.793 3.08v13.904c0 .747.373 1.027 1.214.98l14.523-.84c.841-.046.935-.56.935-1.167V6.354c0-.606-.233-.933-.748-.887l-15.177.887c-.56.047-.747.327-.747.933zm14.337.745c.093.42 0 .84-.42.888l-.7.14v10.264c-.608.327-1.168.514-1.635.514-.748 0-.935-.234-1.495-.933l-4.577-7.186v6.952L12.21 19s0 .84-1.168.84l-3.222.186c-.093-.186 0-.653.327-.746l.84-.233V9.854L7.822 9.76c-.094-.42.14-1.026.793-1.073l3.456-.233 4.764 7.279v-6.44l-1.215-.139c-.093-.514.28-.887.747-.933zM1.936 1.035l13.31-.98c1.634-.14 2.055-.047 3.082.7l4.249 2.986c.7.513.934.653.934 1.213v16.378c0 1.026-.373 1.634-1.68 1.726l-15.458.934c-.98.047-1.448-.093-1.962-.747l-3.129-4.06c-.56-.747-.793-1.306-.793-1.96V2.667c0-.839.374-1.54 1.447-1.632z" />
                                    </svg>
            ...
            <section id="ready" class="ready">
                ...
                <div class="text-container">
                    <p>
                        저는 함께할 준비가 되어 있어요.<br />
                        준비되셨다면, 편하게 메시지를 남겨주세요.
                    </p>
                    <form @submit.prevent="submitForm" novalidate class="multi-step-form">
                        <!-- Step 1 -->
                        <template v-if="step >= 1 && step !== 4">
                            <div class="editor">
                                <label for="name"><span>Q</span> 어떻게 불러드리면 될까요?</label>
                                <div class="answer">
                                    <span>A</span>
                                    <input ref="nameRef" id="name" v-model="form.name" type="text" placeholder="성함 또는 회사" required autocomplete="off" @keydown.enter.prevent :disabled="step !== 1" />
                                </div>
                            </div>
                            <div v-if="step === 1" class="button-wrap">
                                <button type="button" v-hover="{ text: '#fff', bg: '#ce2cfe' }" class="next" v-if="form.name.trim()" @click="nextStep">다음</button>
                            </div>
                        </template>
                        ...
                        <template v-if="step >= 2 && step !== 4">
                            <div class="editor">
                                <label for="contactInput"><span>Q</span> 답변드릴 방법 알려주세요.</label>
                                <div class="toggle-switch">
                                    <button type="button" @click="toggleContactType" class="switch" :disabled="step !== 2">
                                        <span :class="{ active: contactType === 'email' }">이메일</span>
                                        <span :class="{ active: contactType === 'tel' }">전화번호</span>
                                    </button>
                                </div>
                                <div class="answer">
                                    <span>A</span>
                                    <input ref="contactRef" id="contactInput" :type="contactType" v-model="contactValue" :placeholder="contactType === 'email' ? 'justdoit@coding.com' : '010-5959-8282'" required autocomplete="off" @keydown.enter.prevent :disabled="step !== 2" />
                                </div>
                            </div>
                            <div v-if="step === 2" class="button-wrap">
                                <button type="button" v-hover="{ text: '#eeeeee', bg: '#921fc1' }" class="prev" @click="prevStep">이전</button>
                                <button type="button" v-hover="{ text: '#fff', bg: '#ce2cfe' }" class="next" v-if="isContactValid" @click="nextStep">다음</button>
                            </div>
                        </template>
                        ...
                        <template v-if="step >= 3 && step !== 4">
                            <div class="editor">
                                <label for="message"><span>Q</span> 궁금한 점이나 편하게 하고 싶은 얘기를 적어주세요.</label>
                                <div class="answer">
                                    <span>A</span>
                                    <textarea ref="messageRef" id="message" v-model="form.message" placeholder="자유롭게 적어주세요." required autocomplete="off" rows="1" @input="autoResize" @keydown.enter.prevent :disabled="step !== 3" />
                                </div>
                            </div>
                            <div class="button-wrap">
                                <button type="button" v-hover="{ text: '#eeeeee', bg: '#921fc1' }" class="prev" @click="prevStep">이전</button>
                                <button type="button" v-hover="{ text: '#fff', bg: '#ce2cfe' }" class="next" v-if="form.message.trim()" @click="nextStep">다음</button>
                            </div>
                        </template>
                        ...
                        <template v-if="step >= 4">
                            ...
                            <!-- 제출 버튼 -->
                            <div class="button-wrap">
                                <button type="button" v-hover="{ text: '#eeeeee', bg: '#921fc1' }" class="prev last" @click="prevStep">이전</button>
                                <button type="submit" v-hover="{ text: '#fff', bg: '#ce2cfe' }" class="submit">메시지 남기고 답변 기다리기</button>
                                <div class="success-group">
                                    <p class="success-message">메시지 잘 받았어요! 답변 조금만 기다려 주세요 💌</p>
                                    <button class="prev" @click="formReset">처음으로 가기</button>
                                </div>
                            </div>
                        </template>
                    </form>
...
const morphDirections = ["toY", "toX", "toX", "toY", "toX", "toY", "toY", "toX", "toX", "toY", "toX", "toY"];
...
const animation = () => {
    ...
    // 2) 각 단어 모핑은 개별 ScrollTrigger로 제어 (scrub: false)
    gsap.to(el1, {
        text: "es,",
        duration: 0.5,
        ease: "power1.inOut",
    ...
    YHRTransformTl.to("br.later", { display: "none" }, 0)
        ...
        .to(
            ".here-sentence",
            {
                duration: 500,
                text: {
                    value: "I am. <br/> The one you've been looking for.<br>" + "I believe in the value of collaboration<br>" + "and get better outcomes through clear communication.",
                    delimiter: " ", // "" 글자단위, " " 단어단위
                    innerHTML: true, // HTML 태그(<br>)를 렌더링하도록 허용
                },
            },
            "+=100"
        )
        .to(".ready-sentence", {
            duration: 500,
            text: {
                value: "I adapt quickly to new environments, <br>" + "stay flexible when things change,<br>" + "and I'm always excited to learn new technology.<br>" + "Feel free to get in touch below.",
                delimiter: " ", // "" 글자단위, " " 단어단위
                innerHTML: true, // HTML 태그(<br>)를 렌더링하도록 허용
            },
        });
...
const studyItems = computed(() => {
    ...
    return items.filter((item) => item.properties.category?.select?.name === "study").slice(0, 3); // 최대 3개까지만 반환;
});
/** 날짜 포맷 */
...
//         alert(`메시지 잘 받았어요! 조금만 기다려 주세요, 곧 답변 드리겠습니다. 💌`);
//         form.name = "";
//         form.email = "";
//         form.message = "";
//     } else {
//         alert(`잠시 문제가 생긴 것 같아요. 다시 시도해 주세요. 그래도 문제가 지속되면 아래 방법으로 연락 주세요!
...
/**문의하기 (notion db) */
const step = ref(1);
const contactType = ref("email"); // 타입 제거
...
// Step 이동
// contactValue: 이메일/전화 필드 get/set
...
// 연락처 유효성 검사: contactValue만 검사
const isContactValid = computed(() => {
    const v = contactValue.value.trim();
    if (!v) return false;
    return contactType.value === "email" ? /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) : /^\d[\d\s-]{3,}\d$/.test(v);
});
...
const siteKey = "6LdPLGIrAAAAACiuYxdJr6IYnP_wDw46YS-IWl51"; // reChaptcha 공개 키 (사이트키)
let widgetId = null;
watch(step, async (newStep) => {
    if (newStep >= 4) {
        await nextTick();
...
const submitForm = async () => {
    ...
    if (!token) {
        alert("로봇이 아님을 체크해주세요.");
        return;
    }

    // 1. submit 버튼 → 로딩 텍스트 전환
    gsap.to(submitEl, {
        opacity: 0,
        duration: 0.2,
        onComplete: () => {
            if (submitEl) submitEl.textContent = "전송 중...";
    ...
    try {
        ...
        const proceed = confirm(`잠시 문제가 생긴 것 같아요. 다시 시도해 주세요.`);
        if (proceed) {
...
const formReset = () => {
    ...
    resetTl.call(() => {
        if (submitEl) {
            submitEl.textContent = "메시지 남기고 답변 기다리기";
